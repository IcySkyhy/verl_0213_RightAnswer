...
def default_compute_score(data_source, solution_str, ground_truth, extra_info=None, sandbox_fusion_url=None, concurrent_semaphore=None):
    ...
    # Normalize return format to always include 'acc' field
    if isinstance(res, dict):
        # If dict but no 'acc', infer from score (for 0/1 binary rewards)
        if "acc" not in res:
        ...
        return {
            "score": score,
            "acc": score == 1.0,  # For binary 0/1 rewards, 1.0 means correct
        }
    else:
        score = float(res[0])
        return {
