...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        # # 步骤 2: 在 FSDP 包装之前，根据 requires_grad 状态创建优化器
        # if role == "actor" and optim_config is not None:
        #     from verl.utils.torch_functional import get_constant_schedule_with_warmup, get_cosine_schedule_with_warmup

        #     # 从原始的、未被FSDP包装的模型中筛选可训练参数
        #     trainable_params = [p for p in actor_module.parameters() if p.requires_grad]
            
        #     if not trainable_params:
        #         raise ValueError("CRITICAL: The model has no trainable parameters after applying LoRA but before FSDP wrapping.")
            
        #     if self.rank == 0:
        #         print(f"Found {len(trainable_params)} trainable parameter tensors. Creating optimizer now.")
        ...
        #     log_gpu_memory_usage(f"After {role} optimizer init (before FSDP)", logger=logger)
        
        # # 步骤 3: 现在用 FSDP 包装已经与优化器关联的模型
        ...
        if role == "actor" and optim_config is not None:
            ...
            # 加一个断言，如果找不到可训练参数就直接报错，避免“静默失败”
            if not trainable_params:
                raise ValueError(
                    "The model has no trainable parameters after applying LoRA and FSDP. "
                    "This will lead to grad_norm=0. Please check the model and LoRA configuration."
                )
            
            if torch.distributed.get_rank() == 0:
                print(f"Found {len(trainable_params)} trainable parameter tensors for the optimizer.")
